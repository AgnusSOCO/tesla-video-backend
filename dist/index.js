...
async function createTelegramSession(session) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(telegramSessions).values(session);
}
async function getTelegramSessionByToken(authToken) {
  const db = await getDb();
  if (!db) return void 0;
  const result = await db.select().from(telegramSessions).where(eq(telegramSessions.authToken, authToken)).limit(1);
  return result.length > 0 ? result[0] : void 0;
}
async function getUserVideos(userId) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(videos).where(and(eq(videos.userId, userId), eq(videos.status, "ready"))).orderBy(desc(videos.createdAt));
}
...
var appRouter = router({
  ...
  auth: router({
    ...
    checkAuthStatus: publicProcedure.input(z2.object({ authToken: z2.string() })).query(async ({ input }) => {
  ...
  videos: router({
    ...
    get: protectedProcedure.input(z2.object({ videoId: z2.number() })).query(async ({ ctx, input }) => {
    ...
    // Delete video
    delete: protectedProcedure.input(z2.object({ videoId: z2.number() })).mutation(async ({ ctx, input }) => {
      const video = await getVideoById(input.videoId, ctx.user.id);
      if (!video) {
        throw new TRPCError3({
          code: "NOT_FOUND",
    ...
    requestDownload: protectedProcedure.input(
      z2.object({
